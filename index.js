...
let index = {
  verifyPpuType: (ppu) => {
    ...
    // New Auto Format
    if (!_.includes(ppu, '0') && isAutoMovil (ppu)) {
...
function isAutoMovil(ppu) {
  // Validate format XX1111 or XXXX11
  const regexAutomovil = new RegExp(/^[a-z]{2}[\.\- ]?[0-9]{2}[\.\- ]?[0-9]{2}$|[b-d,f-h,j-l,p,r-t,v-z]{2}[\-\. ]?[b-d,f-h,j-l,p,r-t,v-z]{2}[\.\- ]?[0-9]{2}$/, 'i');
